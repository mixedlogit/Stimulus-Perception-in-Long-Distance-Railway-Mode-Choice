...
database1 <-  read.csv('database.hsr_valid.csv',header = T)

database <- rbind(database,database1)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  R[['car']]  =  asc_car + b_mu*(-
                                      ...
                                      log(1 + exp(b_tt/b_mu*(TT_HSR - TT_CAR)/(TT_CAR**b_fi)))*HSR_AV -
                                      log(1 + exp(b_tt/b_mu*(TT_AIR - TT_CAR)/(TT_CAR**b_fi)))*AIR_AV) +
    b_mu*(-log(1 + exp(1/b_mu*(b_co*FA_BUS - b_cocar*CO_CAR)/(CO_CAR**b_fi)))*BUS_AV -
               log(1 + exp(1/b_mu*(b_co*FA_HSR - b_cocar*CO_CAR)/(CO_CAR**b_fi)))*HSR_AV -
               log(1 + exp(1/b_mu*(b_coair*FA_AIR - b_cocar*CO_CAR)/(CO_CAR**b_fi)))*AIR_AV) + 
    b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3_car*INC_3  
  
  R[['bus']]  = asc_bus + b_mu*(-
                                     log(1 + exp(b_tt/b_mu*(TT_CAR - TT_BUS)/(TT_BUS**b_fi)))*DRIV_LIC -
                                     log(1 + exp(b_tt/b_mu*(TT_HSR - TT_BUS)/(TT_BUS**b_fi)))*HSR_AV -
                                     log(1 + exp(b_tt/b_mu*(TT_AIR - TT_BUS)/(TT_BUS**b_fi)))*AIR_AV) +
    b_mu*(-log(1 + exp(1/b_mu*(b_cocar*CO_CAR - b_co*FA_BUS)/(FA_BUS**b_fi)))*DRIV_LIC -
               log(1 + exp(1/b_mu*(b_co*FA_HSR - b_co*FA_BUS)/(FA_BUS**b_fi)))*HSR_AV -
               log(1 + exp(1/b_mu*(b_coair*FA_AIR - b_co*FA_BUS)/(FA_BUS**b_fi)))*AIR_AV) +
    b_mu*(-log(1 + exp(b_fr/b_mu*(FR_HSR - FR_BUS)/(FR_BUS**b_fi)))*HSR_AV -
               log(1 + exp(b_fr/b_mu*(FR_AIR - FR_BUS)/(FR_BUS**b_fi)))*AIR_AV) +
    b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3  
  
  R[['hsr']] = asc_hsr + b_mu*(-
                                    log(1 + exp(b_tt/b_mu*(TT_CAR - TT_HSR)/(TT_HSR**b_fi)))*DRIV_LIC -
                                    log(1 + exp(b_tt/b_mu*(TT_BUS - TT_HSR)/(TT_HSR**b_fi)))*BUS_AV -
                                    log(1 + exp(b_tt/b_mu*(TT_AIR - TT_HSR)/(TT_HSR**b_fi)))*AIR_AV) +
    b_mu*(-log(1 + exp(1/b_mu*(b_cocar*CO_CAR - b_co*FA_HSR)/(FA_HSR**b_fi)))*DRIV_LIC -
               log(1 + exp(1/b_mu*(b_co*FA_BUS - b_co*FA_HSR)/(FA_HSR**b_fi)))*BUS_AV -
               log(1 + exp(1/b_mu*(b_coair*FA_AIR - b_co*FA_HSR)/(FA_HSR**b_fi)))*AIR_AV) +
    b_mu*(-log(1 + exp(b_fr/b_mu*(FR_BUS - FR_HSR)/(FR_HSR**b_fi)))*BUS_AV -
               log(1 + exp(b_fr/b_mu*(FR_AIR - FR_HSR)/(FR_HSR**b_fi)))*AIR_AV)
  
  R[['air']] = asc_air + b_mu*(-
                                    log(1 + exp(b_tt/b_mu*(TT_CAR - TT_AIR)/(TT_AIR**b_fi)))*DRIV_LIC -
                                    log(1 + exp(b_tt/b_mu*(TT_BUS - TT_AIR)/(TT_AIR**b_fi)))*BUS_AV -
                                    log(1 + exp(b_tt/b_mu*(TT_HSR - TT_AIR)/(TT_AIR**b_fi)))*HSR_AV) +
    b_mu*(-log(1 + exp((b_cocar*CO_CAR - b_coair*FA_AIR)/(FA_AIR**b_fi)))*DRIV_LIC -
               log(1 + exp(1/b_mu*(b_co*FA_BUS - b_coair*FA_AIR)/(FA_AIR**b_fi)))*BUS_AV -
               log(1 + exp(1/b_mu*(b_co*FA_HSR - b_coair*FA_AIR)/(FA_AIR**b_fi)))*HSR_AV) +
    b_mu*(-log(1 + exp(b_fr/b_mu*(FR_BUS - FR_AIR)/(FR_AIR**b_fi)))*BUS_AV -
               log(1 + exp(b_fr/b_mu*(FR_HSR - FR_AIR)/(FR_AIR**b_fi)))*HSR_AV) +
