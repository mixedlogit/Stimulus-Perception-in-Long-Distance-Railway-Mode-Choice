...
### Set core controls
apollo_control = list(
  modelName    = 'mnl_crrmPLGW 3',
  modelDescr   = "MNL CRRM model",
  indivID      = "ID",
  outputDirectory = "C:\\Users\\calde\\Google Drive\\Mestrado 2019_Gabriel Caldeira\\Artigos\\6_Mixed model - SP Rail/Model/Second Paper/rrm/HSR/L"
)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  R[['car']]  =  asc_car - 
    log(1 + exp(b_tt*(TT_BUS - TT_CAR)/(TT_CAR**b_fi_tt)))*BUS_AV -
    log(1 + exp(b_tt*(TT_HSR - TT_CAR)/(TT_CAR**b_fi_tt)))*HSR_AV -
    log(1 + exp(b_tt*(TT_AIR - TT_CAR)/(TT_CAR**b_fi_tt)))*AIR_AV -
    log(1 + exp((b_co*FA_BUS - b_cocar*CO_CAR)/(CO_CAR**b_fi_co)))*BUS_AV -
    log(1 + exp((b_co*FA_HSR - b_cocar*CO_CAR)/(CO_CAR**b_fi_co)))*HSR_AV -
    log(1 + exp((b_coair*FA_AIR - b_cocar*CO_CAR)/(CO_CAR**b_fi_co)))*AIR_AV + 
    b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3_car*INC_3  
  
  R[['bus']]  = asc_bus -
    log(1 + exp(b_tt*(TT_CAR - TT_BUS)/(TT_BUS**b_fi_tt)))*DRIV_LIC -
    log(1 + exp(b_tt*(TT_HSR - TT_BUS)/(TT_BUS**b_fi_tt)))*HSR_AV -
    log(1 + exp(b_tt*(TT_AIR - TT_BUS)/(TT_BUS**b_fi_tt)))*AIR_AV -
    log(1 + exp((b_cocar*CO_CAR - b_co*FA_BUS)/(FA_BUS**b_fi_co)))*DRIV_LIC -
    log(1 + exp((b_co*FA_HSR - b_co*FA_BUS)/(FA_BUS**b_fi_co)))*HSR_AV -
    log(1 + exp((b_coair*FA_AIR - b_co*FA_BUS)/(FA_BUS**b_fi_co)))*AIR_AV -
    log(1 + exp(b_fr*(FR_HSR - FR_BUS)/(FR_BUS**b_fi_fr)))*HSR_AV -
    log(1 + exp(b_fr*(FR_AIR - FR_BUS)/(FR_BUS**b_fi_fr)))*AIR_AV + 
    b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3  
  
  R[['hsr']] = asc_hsr - 
    log(1 + exp(b_tt*(TT_CAR - TT_HSR)/(TT_HSR**b_fi_tt)))*DRIV_LIC -
    log(1 + exp(b_tt*(TT_BUS - TT_HSR)/(TT_HSR**b_fi_tt)))*BUS_AV -
    log(1 + exp(b_tt*(TT_AIR - TT_HSR)/(TT_HSR**b_fi_tt)))*AIR_AV -
    log(1 + exp((b_cocar*CO_CAR - b_co*FA_HSR)/(FA_HSR**b_fi_co)))*DRIV_LIC -
    log(1 + exp((b_co*FA_BUS - b_co*FA_HSR)/(FA_HSR**b_fi_co)))*BUS_AV -
    log(1 + exp((b_coair*FA_AIR - b_co*FA_HSR)/(FA_HSR**b_fi_co)))*AIR_AV -
    log(1 + exp(b_fr*(FR_BUS - FR_HSR)/(FR_HSR**b_fi_fr)))*BUS_AV -
    log(1 + exp(b_fr*(FR_AIR - FR_HSR)/(FR_HSR**b_fi_fr)))*AIR_AV
  
  R[['air']] = asc_air - 
    log(1 + exp(b_tt*(TT_CAR - TT_AIR)/(TT_AIR**b_fi_tt)))*DRIV_LIC -
    log(1 + exp(b_tt*(TT_BUS - TT_AIR)/(TT_AIR**b_fi_tt)))*BUS_AV -
    log(1 + exp(b_tt*(TT_HSR - TT_AIR)/(TT_AIR**b_fi_tt)))*HSR_AV -
    log(1 + exp((b_cocar*CO_CAR - b_coair*FA_AIR)/(FA_AIR**b_fi_co)))*DRIV_LIC -
    log(1 + exp((b_co*FA_BUS - b_coair*FA_AIR)/(FA_AIR**b_fi_co)))*BUS_AV -
    log(1 + exp((b_co*FA_HSR - b_coair*FA_AIR)/(FA_AIR**b_fi_co)))*HSR_AV -
    log(1 + exp(b_fr*(FR_BUS - FR_AIR)/(FR_AIR**b_fi_fr)))*BUS_AV -
    log(1 + exp(b_fr*(FR_HSR - FR_AIR)/(FR_AIR**b_fi_fr)))*HSR_AV + 
    b_age2_air*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3 
...
dtt_car <- (-model_crrm$estimate[['b_tt']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_BUS - database$TT_CAR))+1) +
  (-model_crrm$estimate[['b_tt']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_HSR - database$TT_CAR))+1) +
...
dco_car <- (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$FA_BUS - database$CO_CAR))+1) +
  (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$FA_HSR - database$CO_CAR))+1) +
  (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$FA_AIR - database$CO_CAR))+1)
...
dtt_bus <- (-model_crrm$estimate[['b_tt']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_CAR - database$TT_BUS))+1) +
  (-model_crrm$estimate[['b_tt']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_HSR - database$TT_BUS))+1) +
...
dco_bus <- (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$CO_CAR - database$FA_BUS))+1) +
  (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$FA_HSR - database$FA_BUS))+1) +
  (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$FA_AIR - database$FA_BUS))+1)
...
dtt_air <- (-model_crrm$estimate[['b_tt']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_CAR - database$TT_AIR))+1) +
  ...
  (-model_crrm$estimate[['b_tt']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_HSR - database$TT_AIR))+1)

dco_air <- (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$CO_CAR - database$FA_AIR))+1) +
  (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$FA_HSR - database$FA_AIR))+1) +
  (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$FA_HSR - database$FA_AIR))+1)

vtts_air <- dtt_air/dco_air*60

#VTTS HSR
dtt_hsr <- (-model_crrm$estimate[['b_tt']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_CAR - database$TT_HSR))+1) +
  (-model_crrm$estimate[['b_tt']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_BUS - database$TT_HSR))+1) +
  (-model_crrm$estimate[['b_tt']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_AIR - database$TT_HSR))+1)

dco_hsr <- (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$CO_CAR - database$FA_HSR))+1) +
  (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$FA_BUS - database$FA_HSR))+1) +
  (-model_crrm$estimate[['b_co']])/(exp((-model_crrm$estimate[['b_co']])*(database$FA_AIR - database$FA_HSR))+1)

vtts_hsr <- dtt_hsr/dco_hsr*60
