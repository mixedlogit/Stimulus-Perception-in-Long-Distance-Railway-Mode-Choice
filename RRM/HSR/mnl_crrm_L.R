...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  R[['car']]  =  asc_car - 
            log(1 + exp(b_tt*(TT_BUS - TT_CAR)))*BUS_AV -
            log(1 + exp(b_tt*(TT_HSR - TT_CAR)))*HSR_AV -
            log(1 + exp(b_tt*(TT_AIR - TT_CAR)))*AIR_AV -
            log(1 + exp((b_co*FA_BUS - b_cocar*CO_CAR)))*BUS_AV -
            log(1 + exp((b_co*FA_HSR - b_cocar*CO_CAR)))*HSR_AV -
            log(1 + exp((b_coair*FA_AIR - b_cocar*CO_CAR)))*AIR_AV + b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3_car*INC_3  
            
  R[['bus']]  = asc_bus -
    log(1 + exp(b_tt*(TT_CAR - TT_BUS)))*DRIV_LIC -
    log(1 + exp(b_tt*(TT_HSR - TT_BUS)))*HSR_AV -
    log(1 + exp(b_tt*(TT_AIR - TT_BUS)))*AIR_AV -
    log(1 + exp((b_cocar*CO_CAR - b_co*FA_BUS)))*DRIV_LIC -
    log(1 + exp((b_co*FA_HSR - b_co*FA_BUS)))*HSR_AV -
    log(1 + exp((b_coair*FA_AIR - b_co*FA_BUS)))*AIR_AV -
    log(1 + exp(b_fr*(FR_HSR - FR_BUS)))*HSR_AV -
    log(1 + exp(b_fr*(FR_AIR - FR_BUS)))*AIR_AV + b_age2*AGE_2 + b_age3*AGE_3 +
      b_inc2*INC_2 + b_inc3*INC_3  
  
  R[['hsr']] = asc_hsr - 
    log(1 + exp(b_tt*(TT_CAR - TT_HSR)))*DRIV_LIC -
    log(1 + exp(b_tt*(TT_BUS - TT_HSR)))*BUS_AV -
    log(1 + exp(b_tt*(TT_AIR - TT_HSR)))*AIR_AV -
    log(1 + exp((b_cocar*CO_CAR - b_co*FA_HSR)))*DRIV_LIC -
    log(1 + exp((b_co*FA_BUS - b_co*FA_HSR)))*BUS_AV -
    log(1 + exp((b_coair*FA_AIR - b_co*FA_HSR)))*AIR_AV -
    log(1 + exp(b_fr*(FR_BUS - FR_HSR)))*BUS_AV -
    log(1 + exp(b_fr*(FR_AIR - FR_HSR)))*AIR_AV

  R[['air']] = asc_air - 
    log(1 + exp(b_tt*(TT_CAR - TT_AIR)))*DRIV_LIC -
    log(1 + exp(b_tt*(TT_BUS - TT_AIR)))*BUS_AV -
    log(1 + exp(b_tt*(TT_HSR - TT_AIR)))*HSR_AV -
    log(1 + exp((b_cocar*CO_CAR - b_coair*FA_AIR)))*DRIV_LIC -
    log(1 + exp((b_co*FA_BUS - b_coair*FA_AIR)))*BUS_AV -
    log(1 + exp((b_co*FA_HSR - b_coair*FA_AIR)))*HSR_AV -
    log(1 + exp(b_fr*(FR_BUS - FR_AIR)))*BUS_AV -
    log(1 + exp(b_fr*(FR_HSR - FR_AIR)))*HSR_AV + b_age2_air*AGE_2 + b_age3*AGE_3 +
      b_inc2*INC_2 + b_inc3*INC_3 
