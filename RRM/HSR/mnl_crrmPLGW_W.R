...
### Set core controls
apollo_control = list(
  modelName    = 'mnl_crrm 3',
  modelDescr   = "MNL CRRM model",
  indivID      = "ID",
  outputDirectory = "C:\\Users\\calde\\Google Drive\\Mestrado 2019_Gabriel Caldeira\\Artigos\\6_Mixed model - SP Rail/Model/Second Paper/rrm/HSR/W_PLGW"
)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  R[['car']]  =  asc_car - 
            ...
            log(1 + exp(b_co*((FA_BUS - CO_CAR)/(CO_CAR**b_fi_co))))*BUS_AV -
            log(1 + exp(b_co*((FA_HSR - CO_CAR)/(CO_CAR**b_fi_co))))*HSR_AV -
            log(1 + exp(b_co*((FA_AIR - CO_CAR)/(CO_CAR**b_fi_co))))*AIR_AV + b_age2_car*AGE_2 + b_age3_car*AGE_3 +
  ...
  R[['bus']]  = asc_bus -
    ...
    log(1 + exp(b_co*((CO_CAR - FA_BUS)/(FA_BUS**b_fi_co))))*DRIV_LIC -
    log(1 + exp(b_co*((FA_HSR - FA_BUS)/(FA_BUS**b_fi_co))))*HSR_AV -
    log(1 + exp(b_co*((FA_AIR - FA_BUS)/(FA_BUS**b_fi_co))))*AIR_AV -
    log(1 + exp(b_fr*((FR_HSR - FR_BUS)/(FR_BUS**b_fi_fr))))*HSR_AV -
    log(1 + exp(b_fr*((FR_AIR - FR_BUS)/(FR_BUS**b_fi_fr))))*AIR_AV + b_age2*AGE_2 + b_age3_bus*AGE_3 +
  ...
  R[['hsr']] = asc_hsr - 
    ...
    log(1 + exp(b_co*((CO_CAR - FA_HSR)/(FA_HSR**b_fi_co))))*DRIV_LIC -
    log(1 + exp(b_co*((FA_BUS - FA_HSR)/(FA_HSR**b_fi_co))))*BUS_AV -
    log(1 + exp(b_co*((FA_AIR - FA_HSR)/(FA_HSR**b_fi_co))))*AIR_AV -
  ...
  R[['air']] = asc_air - 
    ...
    log(1 + exp(b_co*((CO_CAR - FA_AIR)/(FA_AIR**b_fi_co))))*DRIV_LIC -
    log(1 + exp(b_co*((FA_BUS - FA_AIR)/(FA_AIR**b_fi_co))))*BUS_AV -
    log(1 + exp(b_co*((FA_HSR - FA_AIR)/(FA_AIR**b_fi_co))))*HSR_AV -
    log(1 + exp(b_fr*((FR_BUS - FR_AIR)/(FR_AIR**b_fi_fr))))*BUS_AV -
    log(1 + exp(b_fr*((FR_HSR - FR_AIR)/(FR_AIR**b_fi_fr))))*HSR_AV + b_age2*AGE_2 + b_age3_air*AGE_3 +
