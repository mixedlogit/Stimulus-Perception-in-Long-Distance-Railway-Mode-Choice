...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  R[['car']]  =  asc_car + b_mu*(- 
            ...
            log(1 + exp(b_tt/b_mu*((TT_HSR - TT_CAR)/(TT_CAR**b_fi_tt))))*HSR_AV -
            log(1 + exp(b_tt/b_mu*((TT_AIR - TT_CAR)/(TT_CAR**b_fi_tt))))*AIR_AV -
            log(1 + exp(b_co/b_mu*((FA_BUS - CO_CAR)/(CO_CAR**b_fi_co))))*BUS_AV -
            log(1 + exp(b_co/b_mu*((FA_HSR - CO_CAR)/(CO_CAR**b_fi_co))))*HSR_AV -
            log(1 + exp(b_co/b_mu*((FA_AIR - CO_CAR)/(CO_CAR**b_fi_co))))*AIR_AV) + 
    b_age2_car*AGE_2 + b_age3_car*AGE_3 +
    b_inc2*INC_2 + b_inc3_car*INC_3  
            
  R[['bus']]  = asc_bus + b_mu*(- 
    log(1 + exp(b_tt/b_mu*((TT_CAR - TT_BUS)/(TT_BUS**b_fi_tt))))*DRIV_LIC -
    log(1 + exp(b_tt/b_mu*((TT_HSR - TT_BUS)/(TT_BUS**b_fi_tt))))*HSR_AV -
    log(1 + exp(b_tt/b_mu*((TT_AIR - TT_BUS)/(TT_BUS**b_fi_tt))))*AIR_AV -
    log(1 + exp(b_co/b_mu*((CO_CAR - FA_BUS)/(FA_BUS**b_fi_co))))*DRIV_LIC -
    log(1 + exp(b_co/b_mu*((FA_HSR - FA_BUS)/(FA_BUS**b_fi_co))))*HSR_AV -
    log(1 + exp(b_co/b_mu*((FA_AIR - FA_BUS)/(FA_BUS**b_fi_co))))*AIR_AV -
    log(1 + exp(b_fr/b_mu*((FR_HSR - FR_BUS)/(FR_BUS**b_fi_fr))))*HSR_AV -
    log(1 + exp(b_fr/b_mu*((FR_AIR - FR_BUS)/(FR_BUS**b_fi_fr))))*AIR_AV) + 
    b_age2*AGE_2 + b_age3_bus*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3  
  
  R[['hsr']] = asc_hsr + b_mu*(- 
    log(1 + exp(b_tt/b_mu*((TT_CAR - TT_HSR)/(TT_HSR**b_fi_tt))))*DRIV_LIC -
    log(1 + exp(b_tt/b_mu*((TT_BUS - TT_HSR)/(TT_HSR**b_fi_tt))))*BUS_AV -
    log(1 + exp(b_tt/b_mu*((TT_AIR - TT_HSR)/(TT_HSR**b_fi_tt))))*AIR_AV -
    log(1 + exp(b_co/b_mu*((CO_CAR - FA_HSR)/(FA_HSR**b_fi_co))))*DRIV_LIC -
    log(1 + exp(b_co/b_mu*((FA_BUS - FA_HSR)/(FA_HSR**b_fi_co))))*BUS_AV -
    log(1 + exp(b_co/b_mu*((FA_AIR - FA_HSR)/(FA_HSR**b_fi_co))))*AIR_AV -
    log(1 + exp(b_fr/b_mu*((FR_BUS - FR_HSR)/(FR_HSR**b_fi_fr))))*BUS_AV -
    log(1 + exp(b_fr/b_mu*((FR_AIR - FR_HSR)/(FR_HSR**b_fi_fr))))*AIR_AV)
  
  
  R[['air']] = asc_air + b_mu*(- 
    log(1 + exp(b_tt/b_mu*((TT_CAR - TT_AIR)/(TT_AIR**b_fi_tt))))*DRIV_LIC -
    log(1 + exp(b_tt/b_mu*((TT_BUS - TT_AIR)/(TT_AIR**b_fi_tt))))*BUS_AV -
    log(1 + exp(b_tt/b_mu*((TT_HSR - TT_AIR)/(TT_AIR**b_fi_tt))))*HSR_AV -
    log(1 + exp(b_co/b_mu*((CO_CAR - FA_AIR)/(FA_AIR**b_fi_co))))*DRIV_LIC -
    log(1 + exp(b_co/b_mu*((FA_BUS - FA_AIR)/(FA_AIR**b_fi_co))))*BUS_AV -
    log(1 + exp(b_co/b_mu*((FA_HSR - FA_AIR)/(FA_AIR**b_fi_co))))*HSR_AV -
    log(1 + exp(b_fr/b_mu*((FR_BUS - FR_AIR)/(FR_AIR**b_fi_fr))))*BUS_AV -
    log(1 + exp(b_fr/b_mu*((FR_HSR - FR_AIR)/(FR_AIR**b_fi_fr))))*HSR_AV) + 
    b_age2*AGE_2 + b_age3_air*AGE_3 +
