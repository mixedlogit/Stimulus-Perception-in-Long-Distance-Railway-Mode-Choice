...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  R[['car']]  =  asc_car - 
            ...
            log(1 + exp(b_tt*((TT_CR - TT_CAR)/(TT_CAR**b_fi_tt))))*CR_AV -
            log(1 + exp(b_tt*((TT_AIR - TT_CAR)/(TT_CAR**b_fi_tt))))*AIR_AV -
            log(1 + exp(b_co*((FA_BUS - CO_CAR)/(CO_CAR**b_fi_co))))*BUS_AV -
            log(1 + exp((b_cocr*FA_CR - b_co*CO_CAR)/(CO_CAR**b_fi_co)))*CR_AV -
            log(1 + exp((b_coair*FA_AIR - b_co*CO_CAR)/(CO_CAR**b_fi_co)))*AIR_AV + b_age2_car*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3_car*INC_3
            
  R[['bus']]  = asc_bus -
    log(1 + exp(b_tt*((TT_CAR - TT_BUS)/(TT_BUS**b_fi_tt))))*DRIV_LIC -
    log(1 + exp(b_tt*((TT_CR - TT_BUS)/(TT_BUS**b_fi_tt))))*CR_AV -
    log(1 + exp(b_tt*((TT_AIR - TT_BUS)/(TT_BUS**b_fi_tt))))*AIR_AV -
    log(1 + exp(b_co*((CO_CAR - FA_BUS)/(FA_BUS**b_fi_co))))*DRIV_LIC -
    log(1 + exp((b_cocr*FA_CR - b_co*FA_BUS)/(FA_BUS**b_fi_co)))*CR_AV -
    log(1 + exp((b_coair*FA_AIR - b_coair*FA_BUS)/(FA_BUS**b_fi_co)))*AIR_AV -
    log(1 + exp(b_fr*((FR_CR - FR_BUS)/(FR_BUS**b_fi_fr))))*CR_AV -
    log(1 + exp(b_fr*((FR_AIR - FR_BUS)/(FR_BUS**b_fi_fr))))*AIR_AV + 
    b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3
  
  R[['cr']] = asc_cr - 
    log(1 + exp(b_tt*((TT_CAR - TT_CR)/(TT_CR**b_fi_tt))))*DRIV_LIC -
    log(1 + exp(b_tt*((TT_BUS - TT_CR)/(TT_CR**b_fi_tt))))*BUS_AV -
    log(1 + exp(b_tt*((TT_AIR - TT_CR)/(TT_CR**b_fi_tt))))*AIR_AV -
    log(1 + exp((b_co*CO_CAR - b_cocr*FA_CR)/(FA_CR**b_fi_co)))*DRIV_LIC -
    log(1 + exp((b_co*FA_BUS - b_cocr*FA_CR)/(FA_CR**b_fi_co)))*BUS_AV -
    log(1 + exp((b_coair*FA_AIR - b_cocr*FA_CR)/(FA_CR**b_fi_co)))*AIR_AV -
    log(1 + exp(b_fr*((FR_BUS - FR_CR)/(FR_CR**b_fi_fr))))*BUS_AV -
    log(1 + exp(b_fr*((FR_AIR - FR_CR)/(FR_CR**b_fi_fr))))*AIR_AV
  
  
  R[['air']] = asc_air - 
    log(1 + exp(b_tt*((TT_CAR - TT_AIR)/(TT_AIR**b_fi_tt))))*DRIV_LIC -
    log(1 + exp(b_tt*((TT_BUS - TT_AIR)/(TT_AIR**b_fi_tt))))*BUS_AV -
    log(1 + exp(b_tt*((TT_CR - TT_AIR)/(TT_AIR**b_fi_tt))))*HSR_AV -
    log(1 + exp((b_co*CO_CAR - b_coair*FA_AIR)/(FA_AIR**b_fi_co)))*DRIV_LIC -
    log(1 + exp((b_co*FA_BUS - b_coair*FA_AIR)/(FA_AIR**b_fi_co)))*BUS_AV -
    log(1 + exp((b_cocr*FA_CR - b_coair*FA_AIR)/(FA_AIR**b_fi_co)))*HSR_AV -
    log(1 + exp(b_fr*((FR_BUS - FR_AIR)/(FR_AIR**b_fi_fr))))*BUS_AV -
    log(1 + exp(b_fr*((FR_CR - FR_AIR)/(FR_AIR**b_fi_fr))))*HSR_AV + 
    b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3
