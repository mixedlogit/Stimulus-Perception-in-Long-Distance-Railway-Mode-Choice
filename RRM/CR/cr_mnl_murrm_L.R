...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  R[['car']]  =  asc_car + b_mu*( - 
    ...
    b_mu*(-log(1 + exp((b_co*FA_BUS - b_cocar*CO_CAR)))*BUS_AV -
    log(1 + exp(b_co*FA_CR - b_cocar*CO_CAR))*CR_AV -
    log(1 + exp(b_coair*FA_AIR - b_cocar*CO_CAR))*AIR_AV) +
    b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3
  
  R[['bus']]  = asc_bus + b_mu*(
    log(1 + exp(b_tt*(TT_CAR - TT_BUS)))*DRIV_LIC -
    log(1 + exp(b_tt*(TT_CR - TT_BUS)))*CR_AV -
    log(1 + exp(b_tt*(TT_AIR - TT_BUS)))*AIR_AV) +
    b_mu*(-log(1 + exp((b_cocar*CO_CAR - b_co*FA_BUS)))*DRIV_LIC -
    log(1 + exp(b_co*FA_CR - b_co*FA_BUS))*CR_AV -
    log(1 + exp(b_coair*FA_AIR - b_co*FA_BUS))*AIR_AV) +
    b_mu*(-log(1 + exp(b_fr*(FR_CR - FR_BUS)))*CR_AV -
    log(1 + exp(b_fr*(FR_AIR - FR_BUS)))*AIR_AV) + 
    b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3
  
  R[['cr']] = asc_cr - 
    log(1 + exp(b_tt*(TT_CAR - TT_CR)))*DRIV_LIC -
    log(1 + exp(b_tt*(TT_BUS - TT_CR)))*BUS_AV -
    log(1 + exp(b_tt*(TT_AIR - TT_CR)))*AIR_AV -
    log(1 + exp(b_cocar*CO_CAR - b_co*FA_CR))*DRIV_LIC -
    log(1 + exp(b_co*FA_BUS - b_co*FA_CR))*BUS_AV -
    log(1 + exp(b_coair*FA_AIR - b_co*FA_CR))*AIR_AV -
    log(1 + exp(b_fr*(FR_BUS - FR_CR)))*BUS_AV -
    log(1 + exp(b_fr*(FR_AIR - FR_CR)))*AIR_AV
  
  
  R[['air']] = asc_air + b_mu*(- 
    log(1 + exp(b_tt*(TT_CAR - TT_AIR)))*DRIV_LIC -
    log(1 + exp(b_tt*(TT_BUS - TT_AIR)))*BUS_AV -
    log(1 + exp(b_tt*(TT_CR - TT_AIR)))*HSR_AV) +
    b_mu*(-log(1 + exp(b_cocar*CO_CAR - b_coair*FA_AIR))*DRIV_LIC -
    log(1 + exp(b_co*FA_BUS - b_coair*FA_AIR))*BUS_AV -
    log(1 + exp(b_co*FA_CR - b_coair*FA_AIR))*HSR_AV) +
    b_mu*(-log(1 + exp(b_fr*(FR_BUS - FR_AIR)))*BUS_AV -
    log(1 + exp(b_fr*(FR_CR - FR_AIR)))*HSR_AV) + 
