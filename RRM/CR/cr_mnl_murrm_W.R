...
apollo_control = list(
  ...
  indivID      = "ID",
  outputDirectory = "C:\\Users\\calde\\Google Drive\\Mestrado 2019_Gabriel Caldeira\\Artigos\\6_Mixed model - SP Rail/Model/Second Paper/rrm/CR/W"
)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  R[['car']]  =  asc_car + b_mu*(- 
    ...
    log(1 + exp(b_tt/b_mu*(TT_CR - TT_CAR)))*CR_AV -
    log(1 + exp(b_tt/b_mu*(TT_AIR - TT_CAR)))*AIR_AV -
    log(1 + exp(b_co/b_mu*(FA_BUS - CO_CAR)))*BUS_AV -
    log(1 + exp((b_cocr*FA_CR - b_co*CO_CAR)/b_mu))*CR_AV -
    log(1 + exp((b_coair*FA_AIR - b_co*CO_CAR)/b_mu))*AIR_AV) + b_age2_car*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3_car*INC_3
  
  R[['bus']]  = asc_bus + b_mu*(-
    log(1 + exp(b_tt/b_mu*(TT_CAR - TT_BUS)))*DRIV_LIC -
    log(1 + exp(b_tt/b_mu*(TT_CR - TT_BUS)))*CR_AV -
    log(1 + exp(b_tt/b_mu*(TT_AIR - TT_BUS)))*AIR_AV -
    log(1 + exp(b_co/b_mu*(CO_CAR - FA_BUS)))*DRIV_LIC -
    log(1 + exp((b_cocr*FA_CR - b_co*FA_BUS)/b_mu))*CR_AV -
    log(1 + exp((b_coair*FA_AIR - b_coair*FA_BUS)/b_mu))*AIR_AV -
    log(1 + exp(b_fr/b_mu*(FR_CR - FR_BUS)))*CR_AV -
    log(1 + exp(b_fr/b_mu*(FR_AIR - FR_BUS)))*AIR_AV) + b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3
  
  R[['cr']] = asc_cr + b_mu*(- 
    log(1 + exp(b_tt/b_mu*(TT_CAR - TT_CR)))*DRIV_LIC -
    log(1 + exp(b_tt/b_mu*(TT_BUS - TT_CR)))*BUS_AV -
    log(1 + exp(b_tt/b_mu*(TT_AIR - TT_CR)))*AIR_AV -
    log(1 + exp((b_co*CO_CAR - b_cocr*FA_CR)/b_mu))*DRIV_LIC -
    log(1 + exp((b_co*FA_BUS - b_cocr*FA_CR)/b_mu))*BUS_AV -
    log(1 + exp((b_coair*FA_AIR - b_cocr*FA_CR)/b_mu))*AIR_AV -
    log(1 + exp(b_fr/b_mu*(FR_BUS - FR_CR)))*BUS_AV -
    log(1 + exp(b_fr/b_mu*(FR_AIR - FR_CR)))*AIR_AV)
  
  
  R[['air']] = asc_air + b_mu*(- 
    log(1 + exp(b_tt/b_mu*(TT_CAR - TT_AIR)))*DRIV_LIC -
    log(1 + exp(b_tt/b_mu*(TT_BUS - TT_AIR)))*BUS_AV -
    log(1 + exp(b_tt/b_mu*(TT_CR - TT_AIR)))*HSR_AV -
    log(1 + exp((b_co*CO_CAR - b_coair*FA_AIR)/b_mu))*DRIV_LIC -
    log(1 + exp((b_co*FA_BUS - b_coair*FA_AIR)/b_mu))*BUS_AV -
    log(1 + exp((b_cocr*FA_CR - b_coair*FA_AIR)/b_mu))*HSR_AV -
    log(1 + exp(b_fr/b_mu*(FR_BUS - FR_AIR)))*BUS_AV -
    log(1 + exp(b_fr/b_mu*(FR_CR - FR_AIR)))*HSR_AV) + b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3
