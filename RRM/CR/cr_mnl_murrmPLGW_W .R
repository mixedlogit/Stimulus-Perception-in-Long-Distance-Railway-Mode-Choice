...
### Set core controls
apollo_control = list(
  modelName    = 'mnl_murrm 3',
  modelDescr   = "MNL CRRM model",
  indivID      = "ID",
  outputDirectory = "C:\\Users\\calde\\Google Drive\\Mestrado 2019_Gabriel Caldeira\\Artigos\\6_Mixed model - SP Rail/Model/Second Paper/rrm/CR/W_PLGW"
)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  R[['car']]  =  asc_car + b_mu*(- 
    log(1 + exp(b_tt/b_mu*(((TT_BUS - TT_CAR)/(TT_CAR**b_fi_tt))))*BUS_AV -
    log(1 + exp(b_tt/b_mu*(TT_CR - TT_CAR)/(TT_CAR**b_fi_tt))))*CR_AV -
    log(1 + exp(b_tt/b_mu*((TT_AIR - TT_CAR)/(TT_CAR**b_fi_tt))))*AIR_AV -
    log(1 + exp(b_co/b_mu*((FA_BUS - CO_CAR)/(CO_CAR**b_fi_co))))*BUS_AV -
    log(1 + exp(((b_cocr*FA_CR - b_co*CO_CAR)/(CO_CAR**b_fi_co))/b_mu))*CR_AV -
    log(1 + exp(((b_coair*FA_AIR - b_co*CO_CAR)/(CO_CAR**b_fi_co))/b_mu))*AIR_AV) + 
    b_age2_car*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3_car*INC_3
  
  R[['bus']]  = asc_bus + b_mu*(-
    log(1 + exp(b_tt/b_mu*((TT_CAR - TT_BUS)/(TT_BUS**b_fi_tt))))*DRIV_LIC -
    log(1 + exp(b_tt/b_mu*((TT_CR - TT_BUS)/(TT_BUS**b_fi_tt))))*CR_AV -
    log(1 + exp(b_tt/b_mu*((TT_AIR - TT_BUS)/(TT_BUS**b_fi_tt))))*AIR_AV -
    log(1 + exp(b_co/b_mu*((CO_CAR - FA_BUS)/(FA_BUS**b_fi_co))))*DRIV_LIC -
    log(1 + exp(((b_cocr*FA_CR - b_co*FA_BUS)/(FA_BUS**b_fi_co))/b_mu))*CR_AV -
    log(1 + exp(((b_coair*FA_AIR - b_coair*FA_BUS)/(FA_BUS**b_fi_co))/b_mu))*AIR_AV -
    log(1 + exp(b_fr/b_mu*((FR_CR - FR_BUS)/(FR_BUS**b_fi_fr))))*CR_AV -
    log(1 + exp(b_fr/b_mu*((FR_AIR - FR_BUS)/(FR_BUS**b_fi_fr))))*AIR_AV) +
    b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3
  
  R[['cr']] = asc_cr + b_mu*(- 
    log(1 + exp(b_tt/b_mu*((TT_CAR - TT_CR)/(TT_CR**b_fi_tt))))*DRIV_LIC -
    log(1 + exp(b_tt/b_mu*((TT_BUS - TT_CR)/(TT_CR**b_fi_tt))))*BUS_AV -
    log(1 + exp(b_tt/b_mu*((TT_AIR - TT_CR)/(TT_CR**b_fi_tt))))*AIR_AV -
    log(1 + exp(((b_co*CO_CAR - b_cocr*FA_CR)/(FA_CR**b_fi_co))/b_mu))*DRIV_LIC -
    log(1 + exp(((b_co*FA_BUS - b_cocr*FA_CR)/(FA_CR**b_fi_co))/b_mu))*BUS_AV -
    log(1 + exp(((b_coair*FA_AIR - b_cocr*FA_CR)/(FA_CR**b_fi_co))/b_mu))*AIR_AV -
    log(1 + exp(b_fr/b_mu*((FR_BUS - FR_CR)/(FR_CR**b_fi_fr))))*BUS_AV -
    log(1 + exp(b_fr/b_mu*((FR_AIR - FR_CR)/(FR_CR**b_fi_fr))))*AIR_AV)
  
  
  R[['air']] = asc_air + b_mu*(- 
    log(1 + exp(b_tt/b_mu*((TT_CAR - TT_AIR)/(TT_AIR**b_fi_tt))))*DRIV_LIC -
    log(1 + exp(b_tt/b_mu*((TT_BUS - TT_AIR)/(TT_AIR**b_fi_tt))))*BUS_AV -
    log(1 + exp(b_tt/b_mu*((TT_CR - TT_AIR)/(TT_AIR**b_fi_tt))))*HSR_AV -
    log(1 + exp(((b_co*CO_CAR - b_coair*FA_AIR)/(FA_AIR**b_fi_co))/b_mu))*DRIV_LIC -
    log(1 + exp(((b_co*FA_BUS - b_coair*FA_AIR)/(FA_AIR**b_fi_co))/b_mu))*BUS_AV -
    log(1 + exp(((b_cocr*FA_CR - b_coair*FA_AIR)/(FA_AIR**b_fi_co))/b_mu))*HSR_AV -
    log(1 + exp(b_fr/b_mu*((FR_BUS - FR_AIR)/(FR_AIR**b_fi_fr))))*BUS_AV -
    log(1 + exp(b_fr/b_mu*((FR_CR - FR_AIR)/(FR_AIR**b_fi_fr))))*HSR_AV) +
    b_age2*AGE_2 + b_age3*AGE_3 +
    b_inc2*INC_2 + b_inc3*INC_3
...
model_murrm  <-  apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs, 
...
apollo_modelOutput(model_murrm, modelOutput_settings = list(printClassical = TRUE,printPVal = TRUE))

# ----------------------------------------------------------------- #
#---- FORMATTED OUTPUT (TO FILE, using model name)               ----
# ----------------------------------------------------------------- #

apollo_saveOutput(model_murrm,saveOutput_settings = list(printPVal=T ,printT1=T,
...
dtt_car <- (-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])*(database$TT_BUS - database$TT_CAR))+1) +
  (-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])*(database$TT_CR - database$TT_CAR))+1) +
...
dco_car <- (-model_crrm$estimate[['b_co']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_co']]/model_crrm$estimate[['b_mu']])*(database$FA_BUS - database$CO_CAR))+1) +
  (-model_crrm$estimate[['b_co']]/model_crrm$estimate[['b_mu']])/(exp(-(model_crrm$estimate[['b_cocr']]*database$FA_CR - model_crrm$estimate[['b_co']]*database$CO_CAR)/model_crrm$estimate[['b_mu']])+1) +
  (-model_crrm$estimate[['b_co']]/model_crrm$estimate[['b_mu']])/(exp(-(model_crrm$estimate[['b_coair']]*database$FA_AIR - model_crrm$estimate[['b_co']]*database$CO_CAR)/model_crrm$estimate[['b_mu']])+1)
...
dtt_bus <- (-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])*(database$TT_CAR - database$TT_BUS))+1) +
  (-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])*(database$TT_CR - database$TT_BUS))+1) +
...
dco_bus <- (-model_crrm$estimate[['b_co']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_co']]/model_crrm$estimate[['b_mu']])*(database$CO_CAR - database$FA_BUS))+1) +
  (-model_crrm$estimate[['b_co']]/model_crrm$estimate[['b_mu']])/(exp(-(model_crrm$estimate[['b_cocr']]*database$FA_CR - model_crrm$estimate[['b_co']]*database$FA_BUS)/model_crrm$estimate[['b_mu']])+1) +
  (-model_crrm$estimate[['b_co']]/model_crrm$estimate[['b_mu']])/(exp(-(model_crrm$estimate[['b_coair']]*database$FA_AIR - model_crrm$estimate[['b_co']]*database$FA_BUS)/model_crrm$estimate[['b_mu']])+1)

vtts_bus <- dtt_bus/dco_bus*60

#VTTS air
dtt_air <- (-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_CAR - database$TT_AIR)/model_crrm$estimate[['b_mu']])+1) +
  (-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_BUS - database$TT_AIR)/model_crrm$estimate[['b_mu']])+1) +
  (-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_tt']])*(database$TT_CR - database$TT_AIR)/model_crrm$estimate[['b_mu']])+1)

dco_air <- (-model_crrm$estimate[['b_coair']]/model_crrm$estimate[['b_mu']])/(exp(-(model_crrm$estimate[['b_co']]*database$CO_CAR - model_crrm$estimate[['b_coair']]*database$FA_AIR)/model_crrm$estimate[['b_mu']])+1) +
  (-model_crrm$estimate[['b_coair']]/model_crrm$estimate[['b_mu']])/(exp(-(model_crrm$estimate[['b_co']]*database$FA_BUS - model_crrm$estimate[['b_coair']]*database$FA_AIR)/model_crrm$estimate[['b_mu']])+1) +
  (-model_crrm$estimate[['b_coair']]/model_crrm$estimate[['b_mu']])/(exp(-(model_crrm$estimate[['b_cocr']]*database$FA_CR - model_crrm$estimate[['b_coair']]*database$FA_AIR)/model_crrm$estimate[['b_mu']])+1)

vtts_air <- dtt_air/dco_air*60

#VTTS CR
dtt_cr <- (-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])*(database$TT_CAR - database$TT_CR))+1) +
  (-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])*(database$TT_BUS - database$TT_CR))+1) +
  (-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])/(exp((-model_crrm$estimate[['b_tt']]/model_crrm$estimate[['b_mu']])*(database$TT_AIR - database$TT_CR))+1)

dco_cr <- (-model_crrm$estimate[['b_cocr']]/model_crrm$estimate[['b_mu']])/(exp(-(model_crrm$estimate[['b_co']]*database$CO_CAR - model_crrm$estimate[['b_cocr']]*database$FA_CR)/model_crrm$estimate[['b_mu']])+1) +
  (-model_crrm$estimate[['b_cocr']]/model_crrm$estimate[['b_mu']])/(exp(-(model_crrm$estimate[['b_co']]*database$FA_BUS - model_crrm$estimate[['b_cocr']]*database$FA_CR)/model_crrm$estimate[['b_mu']])+1) +
  (-model_crrm$estimate[['b_cocr']]/model_crrm$estimate[['b_mu']])/(exp(-(model_crrm$estimate[['b_coair']]*database$FA_AIR - model_crrm$estimate[['b_cocr']]*database$FA_CR)/model_crrm$estimate[['b_mu']])+1)
