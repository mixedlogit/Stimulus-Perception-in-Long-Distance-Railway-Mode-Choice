...
setwd("C:\\Users\\calde\\Google Drive\\Mestrado 2019_Gabriel Caldeira\\Artigos\\4_Mixed model - SP Rail\\Model\\First Paper\\mnl_rum\\HSR\\L")
...
model_rum <- read_rds("mnl_rum 16_model.rds")

#VTTS RUM
apollo_deltaMethod(model = model_rum,deltaMethod_settings = list(operation="ratio",parName1='b_tt',parName2="b_cocar",multPar1=60))
...
apollo_lrTest('mnl_rum 16','mnl_rum 17')
...
# Confidence intervals of VTTs
apollo_deltaMethod(model = model_rum,
                   deltaMethod_settings = list(expression = c('b_tt/b_co*60'),printPVal=T))

apollo_deltaMethod(model = model_rum,
                   deltaMethod_settings = list(expression = c('b_fr/b_co*60'),printPVal=T))
